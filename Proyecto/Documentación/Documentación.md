...
Según el blog Tips para estudiantes de Sistemas Computacionales (2012), para el procesamiento de imágenes en el formato BMP,  se recibe la dirección de memoria de la imagen, la cual debe estar cargada en el sistema operativo. Posteriormente, es necesario almacenar el archivo en un buffer, y para esto es necesario asegurarse que el archivo BMP es válido y que contiene la información correspondiente, esto se realiza mediante una lectura del archivo, ya que este consta de un encabezado de 54 bytes que contiene toda la información del BMP. Una vez almacenado el archivo en el buffer, este se utiliza para modificar la paleta de colores. Es importante recordar que los colores en un archivo BMP se guardan como BGR y no como RGB. Asimismo, para cargar la imagen, se debe realizar una lectura línea por línea y se despliega comenzando con la línea inferior hasta la superior. La esquina superior izquierda de la imagen se pinta en la esquina superior de la pantalla. 
