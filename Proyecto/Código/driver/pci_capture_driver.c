...
// PCI device offsets
#define PCI_DEVICE_COMMAND_OFFSET 0x14                      // offset of cmd register
#define PCI_DEVICE_WIDTH_OFFSET 0x2a                        // offset of width register
#define PCI_DEVICE_HEIGHT_OFFSET 0x2e                       // offset of height register
#define PCI_DEVICE_IMAGE_SIZE_OFFSET 0x32                   // offset of image size register
#define PCI_DEVICE_ACCESS_BUFFER_OFFSET 0x36                // offset of access buffer register
#define PCI_DEVICE_VALUE_BUFFER_OFFSET 0x3a                 // offset of value buffer register
#define PCI_DEVICE_ACCESS_VALUE_BUFFER_OFFSET 0x3e          // offset of access value buffer register
...
static long ioctl_pci_capture_chr_dev(struct file *file, unsigned int cmd, unsigned long arg) {
    ...
    switch (cmd) {
        ...
        case READ_IMAGE_SIZE_FROM_PCI_DEVICE:
            ioctl_buffer = (int32_t) read_image_size_to_pci_device();
            error = copy_to_user((int32_t *) arg, &ioctl_buffer, sizeof(ioctl_buffer));
            if (error != 0) {
                printk("IOCTL failed while sending data to user. Error: %d\n", error);
            }
        ...
        case WRITE_FOR_RESET:
            write_command_to_pci_device(2);
            error = 0;
            break;

        case OVERWRITE_BUFFER:            
            error = copy_from_user(&ioctl_buffer, (uint32_t *) arg, sizeof(ioctl_buffer));
            if (error != 0) {
                printk("IOCTL write data failed. Error: %d\n", error);
            } else {
                write_access_value_buffer_to_pci_device(ioctl_buffer);
            }
            error = 0;
            break;

        case WRITE_IMAGE:
            write_command_to_pci_device(3);
            error = 0;
            break;
...
void write_access_value_buffer_to_pci_device(uint32_t command) {
    ...
    iowrite32(command, pci_capture_data->hwmem + PCI_DEVICE_ACCESS_VALUE_BUFFER_OFFSET);
}
...
int32_t read_image_size_to_pci_device(void) {
    ...
    value = ioread32(pci_capture_data->hwmem + PCI_DEVICE_IMAGE_SIZE_OFFSET);
    return value;
}
